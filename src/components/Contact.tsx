import React from "react";
import Image from "next/image";

function Contact() {
  return (
    <>
      <div className=" relative w-full h-64 sm:h-56 md:h-64 lg:h-80 bg-cover bg-center overflow-hidden bgImage">
        <div className="text-white px-4 text-justify py-7 lg:pt-20 container md:ml-4 lg:ml-20 md:pt-14 ">
          <h4 className="Montserrat font-extrabold font28 lg:font48">
            CONTACT US
          </h4>
          <p className="Montserrat font-normal font14 md:max-w-80">
            Lorem ipsum dolor sit amet consectetur. Tristique sociis nunc amet
            erat. Aenean magna lectus viverra eros nullam at ac. Et auctor massa
            tellus vestibulum. Posuere facilisis sed nunc elementum condimentum
            sed ultrices. Feugiat ultrices eu risus.{" "}
          </p>
        </div>
      </div>
      {/* Contact Section */}
      {/* <div className="bg-[--bg-body]"> */}
      {/* <section className="relative pb-12  bg-[--bg-body] ">
          <div className=" bg-[--bg-body] relative w-full h-64  sm:h-56 md:h-64 lg:h-72 bg-cover bg-center bgImage">
            <div className="relative container mx-auto px-4 lg:flex lg:items-center lg:justify-between text-white"> */}
      {/* Left Section - Contact Details destop */}
      {/* <div className="lg:w-1/2  relative max-w-[282px] bottom-[30px] md:max-w-[500px] md:ml-[70px] space-y-2 lg:space-y-6  md:bottom-[10px] lg:bottom-[12px] md:top-[40px] top-[17px] lg:top-0">
                <h2 className="font20 md:text-4xl font-extrabold md:mb-4 Montserrat">
                  CONTACT US
                </h2>
                <p className="text-justify mb-8 max-w-[350px] text14 md:text-sm Montserrat">
                  Lorem ipsum dolor sit amet consectetur. Tristique sociis nunc
                  amet erat. Aenean magna lectus viverra eros nullam at ac. Et
                  auctor massa tellus vestibulum. Posuere facilisis sed nunc
                  elementum condimentum sed ultrices. Feugiat ultrices eu risus.{" "}
                </p>
              </div>
            </div>
          </div> */}
      {/* Content */}
      {/* </section> */}
      {/* </div> */}
    </>
  );
}

export default Contact;
